# DBLP paper–venue LP preprocessing (Var1: area-paper)
...
OUT = 'data/preprocessed/DBLP_lp_pc_var1_ct_from_pt/'
RAW = 'data/raw/DBLP/'
...
for p,r in pr[['paper_id','area_id']].itertuples(index=False):
    if p in Pa_map and r in Ar_map:
        pi,ri = A+Pa_map[p], A+P+T+C+Ar_map[r]
        adjM[pi,ri] = 1
        adjM[ri,pi] = 1
...
area_paper_list = {i: adjM[A+P+T+C+i, A:A+P].nonzero()[0] for i in range(R)}
paper_area_list = {i: adjM[A + i, A+P+T+C:].nonzero()[0] for i in range(P)}
...
#1-4-1
print("Starting Metapath: 141")
p_r_p = []
for r, p_list in area_paper_list.items():
...
#     candidate_c2_list = np.random.choice(len(paper_conf_list[p2 - A]), max(1,int(0.2 * len(paper_conf_list[p2 - A]))), replace=False)
#     candidate_c2_list = paper_conf_list[p2 - A][candidate_c2_list]
#     c_p_r_p_c.extend([(c1, p1, r, p2, c2) for c1 in candidate_c1_list for c2 in candidate_c2_list])
# c_p_r_p_c = np.array(c_p_r_p_c)
# c_p_r_p_c[:, [0,4]] += A+P+T
# sorted_index = sorted(list(range(len(c_p_r_p_c))), key=lambda i : c_p_r_p_c[i, [0, 4, 1, 2, 3]].tolist())
# c_p_r_p_c = c_p_r_p_c[sorted_index]
...
# expected_metapaths = [
#     [(1,0,1), (1,2,1), (1,4,1), (1,3,1)],
#     [(3,1,0,1,3), (3,1,2,1,3), (3,1,3), (3,1,4,1,3)]
# ]
expected_metapaths = [
    [(1,0,1), (1,2,1), (1,4,1), (1,3,1)],
