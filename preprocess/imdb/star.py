...
save_prefix = 'data/preprocessed/IMDB_preprocessed_star/'
num_ntypes = 4
...
adjM_da2m = adjM[len(movies):(len(movies) + len(directors) + len(actors)), :len(movies)]
...
adjM_da2m = adjM[:len(movies), len(movies):(len(movies) + len(directors) + len(actors))]
adjM_da2m_normalized = np.diag(1 / adjM_da2m.sum(axis=1)).dot(adjM_da2m)
