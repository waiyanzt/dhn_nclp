...
(generated by KC_scripts/CMPNN/paper-venue_shared_splits.py). Run that first.

Usage:
    python preprocess/preprocess_DBLP_dhn_lp.py --variant v1
    python preprocess/preprocess_DBLP_dhn_lp.py --variant v1,v2
"""
...
def build_typed_edges(variant, pa, pt, pc, pr, train_pos_local, maps):
    ...
    if variant == "v1":
        ...
        # C-R (conf-area): a conf covers an area if any of its papers has it
        cr = (pc[["paper_id", "conf_id"]].drop_duplicates()
              .merge(pr[["paper_id", "area_id"]].drop_duplicates(), on="paper_id")
              [["conf_id", "area_id"]].drop_duplicates())
...
def preprocess_one(raw_dir, shared_npz, variant, out_path, seed):
    set_seed(seed)
...
def main():
    ...
    if not Path(args.shared_npz).is_file():
        raise SystemExit(f"Missing shared npz: {args.shared_npz}\n"
                         f"Run KC_scripts/CMPNN/paper-venue_shared_splits.py first.")

    for v in _parse_variants(args.variant):
        out_path = f"{args.out_dir}/DBLP_dhn_lp_pc_{v}.pt"
        preprocess_one(args.raw_dir, args.shared_npz, v, out_path, args.seed)
