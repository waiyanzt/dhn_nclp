"""Joint IMDb graph-variant augmentation for DHN node classification.

One DHN, optimizer, and checkpoint are shared across IMDb v1-v4. A
*super-epoch* is one full-batch optimizer update on every selected variant.
Checkpoint selection uses mean validation Macro-F1 after the balanced
super-epoch, matching the RGCN augmentation experiment contract while retaining
the DHN architecture, loss, and optimizer from the supplied DHN config.
...
from experiments.node_classification.train import resolve_layers_config


BUNDLE_PATHS = {
    "v1": Path("data/preprocessed/IMDB_dhn_nc.pt"),
    "v2": Path("data/preprocessed/IMDB_dhn_nc_t.pt"),
    "v3": Path("data/preprocessed/IMDB_dhn_nc_t_2.pt"),
    "v4": Path("data/preprocessed/IMDB_dhn_nc_t_3.pt"),
    "universal": Path("data/preprocessed/IMDB_dhn_nc_universal.pt"),
}
DEFAULT_VARIANTS = ("v1", "v2", "v3", "v4")
VARIANT_ALIASES = {
    "imdb1": "v1",
    "imdb2": "v2",
    "imdb3": "v3",
...
def parse_variants(value: str) -> list[str]:
    variants = [
        VARIANT_ALIASES.get(item.lower(), item.lower()) for item in parse_csv(value)
    ]
    unknown = sorted(set(variants) - set(BUNDLE_PATHS))
    if unknown:
        raise ValueError(f"Unknown IMDb variants: {unknown}")
...
def prepare_bundles(
    ...
    for variant in variants:
        ...
        if bundle.get("meta", {}).get("variant") != variant:
            raise ValueError(
                f"{path} identifies variant "
                f"{bundle.get('meta', {}).get('variant')!r}, expected {variant!r}"
            )
    ...
    assert_same_tensor(
        "features", {variant: bundle["data"].x for variant, bundle in bundles.items()}
    )
    assert_same_tensor(
...
def validate_model_bundle_contract(
    ...
    for layer_number, layer in enumerate(configured_layers, start=1):
        ...
        if kernels != patterns:
            raise ValueError(
                f"Layer {layer_number} kernels {sorted(kernels)} do not match "
...
def create_model(
    config: Mapping[str, Any], num_features: int, num_classes: int
) -> DHN:
...
def run_seed(
    ...
    reference = bundles[variants[0]]
    model = create_model(
        config, int(reference["num_features"]), int(reference["num_classes"])
    ).to(device)
    ...
    patience = (
        ...
        else int(training_config.get("patience", 40))
    )
    grad_clip = (
        args.grad_clip
    ...
    early_stopper = EarlyStopper("max", patience, min_delta=1e-6)
    ...
    for super_epoch in range(super_epochs_ran, super_epochs):
        ...
        mean_val_macro_f1 = float(
            np.mean(
                [
                    validation_metrics[variant]["Macro_F1"]
                    for variant in variants
                ]
            )
        )
        ...
        if early_stopper.update(mean_val_macro_f1):
            atomic_torch_save(
                {
    ...
    summary = {
        ...
        "epoch_accounting": {
            "definition": (
                "one super-epoch is one full-batch optimizer update on "
                "every selected graph variant"
            ),
...
def parse_bundle_overrides(values: list[str]) -> dict[str, Path]:
    ...
    for value in values:
        ...
        canonical = VARIANT_ALIASES.get(variant.lower(), variant.lower())
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Joint IMDb graph-variant augmentation for DHN"
    )
    parser.add_argument("--config", type=Path, default=Path("configs/imdb_nc.yaml"))
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Override a bundle path, e.g. --bundle v1=/path/IMDB_dhn_nc.pt",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/dhn_augmentation/IMDB"),
    )
    parser.add_argument(
        "--super-epochs", "--epochs",
        dest="super_epochs", type=int, default=None,
    )
    ...
    if args.preflight_only:
        ...
        print(
            f"[OK] IMDb joint augmentation preflight: variants={variants}, "
            f"nodes={bundles[variants[0]]['data'].num_nodes}, "
            f"features={bundles[variants[0]]['num_features']}, seeds={seeds}"
        )
        for variant in variants:
